...
export default function IntegrationsPage() {
  ...
  const { data: salesforceData, mutate: mutateSalesforce } = useSWR(
  ...
  const handleConnect = async (integration: Integration) => {
    ...
    try {
      if (integration.id === 'hubspot') {
        ...
        alert(
          `To enable ${integration.name}, add ${integration.configKey} to your environment variables in Vercel.`
        )
      }
    } catch (error) {
  ...
  const handleDisconnect = async (integration: Integration) => {
    ...
    if (['openai', 'plaid', 'stripe', 'resend', 'slack'].includes(integration.id)) {
      alert(
        `${integration.name} is configured via environment variables. To disconnect, remove ${integration.configKey} from your Vercel environment settings.`
      )
      return
    }
                    ...
                    <div className="flex items-center gap-4">
                      <div
                        className={`p-3 rounded-lg ${
                          status === 'connected' ? 'bg-emerald-500/10' : 'bg-secondary'
                        }`}
                      >
                        <integration.icon
                          className={`h-6 w-6 ${
