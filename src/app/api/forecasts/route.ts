...
export async function GET(request: Request) {
  try {
    ...
    const validConfidence = [0.9, 0.95, 0.99].includes(confidence) ? confidence : 0.95
    ...
    const previousMRR =
      historicalData.length > 1 ? historicalData[historicalData.length - 2].value : currentMRR
