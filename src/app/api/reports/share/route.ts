...
export async function POST(req: NextRequest) {
    try {
        ...
        const reportData = {
            ...
            summary: 'This period shows strong financial performance with MRR growing 12.5% month-over-month. Cash runway remains healthy at 9.9 months. Key drivers include new customer acquisition and reduced churn.'
        };
...
export async function DELETE(req: NextRequest) {
    try {
        ...
        // Verify ownership
        if (sharedReport.createdBy !== session.user.email && sharedReport.createdBy !== session.user.id) {
