...
export function MetricCard({
  ...
  const TrendIcon =
    effectiveTrend === 'up' ? TrendingUp : effectiveTrend === 'down' ? TrendingDown : Minus
  const trendColor = (() => {
    if (type === 'runway') {
      // For runway, up is good
      return effectiveTrend === 'up'
        ? 'text-emerald-500'
        : effectiveTrend === 'down'
          ? 'text-red-500'
          : 'text-gray-400'
  ...
  const runwayStatus =
    type === 'runway' && typeof value === 'number' ? getRunwayStatus(value) : null
  ...
  return (
    ...
    >
      ...
      <CardContent className={cn('relative', size === 'lg' ? 'p-8' : 'p-6')}>
        {/* Header with icon */}
          ...
          {runwayStatus && (
            <span
              className={cn(
                ...
                runwayStatus.status === 'caution' &&
                  'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400',
                runwayStatus.status === 'healthy' &&
          ...
          <div className="flex items-center gap-1 mt-3">
            ...
            {change !== undefined && (
              <span className={cn('text-sm font-medium', trendColor)}>
                {change > 0 ? '+' : ''}
                {type === 'currency' ? formatCurrency(change) : formatPercentage(change)}
              </span>
            )}
            {changeLabel && (
