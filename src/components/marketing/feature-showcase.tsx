...
const features = [
  {
    ...
    description:
      'Stop waiting for month-end reports. Get a live pulse on your burn rate, runway, and cash position instantly via Plaid integration.',
    icon: Zap,
          ...
          <div className="h-32 w-64 mx-auto mt-8 flex items-end justify-center gap-2">
            {[40, 65, 50, 80, 60, 90, 75].map((h, i) => (
              <div
                key={i}
                style={{ height: `${h}%` }}
                className="w-8 bg-indigo-500 rounded-t-sm opacity-80"
              />
            ))}
          </div>
        </div>
  ...
  },
  {
    ...
    description:
      'Simulate hiring plans, marketing spend, and product launches. See exactly how decisions impact your runway in real-time.',
    icon: BarChart3,
    component: (
  ...
  },
  {
    ...
    description:
      "Ask complex questions like 'Why did burn increase last month?' and get data-backed answers with citations to specific transactions.",
    icon: MessageSquare,
    component: (
        ...
        <div className="flex-1 space-y-4">
          ...
          <div className="flex gap-3 justify-start">
            <div className="bg-zinc-800 text-zinc-300 px-4 py-3 rounded-2xl rounded-tl-sm text-sm border border-white/5 shadow-xl">
              <p>
                Driven by a <strong>$12k increase in cloud costs</strong> (AWS) and the new{' '}
                <strong>Q3 marketing retainer</strong>.
              </p>
