...
export class ProphetService {
   ...
   */
  forecast(historicalData: DataPoint[], options: ForecastOptions = {}): ForecastResult {
    const { horizonMonths = 12, confidenceLevel = 0.95, seasonalPeriod = 12 } = options
    ...
    for (let t = 0; t < n; t++) {
      ...
      if (useSeasonality) {
        seasonal[seasonalIdx] =
          this.gamma * (data[t] / level) + (1 - this.gamma) * seasonal[seasonalIdx]
      }
    }
