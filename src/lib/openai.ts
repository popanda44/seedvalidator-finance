...
Respond only with valid JSON.`
  ...
  const response = await getOpenAIClient().chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a startup financial advisor. Always respond in valid JSON format.',
      },
...
Provide a clear, concise answer in 2-3 sentences. Be specific with numbers when relevant.`
  ...
  const response = await getOpenAIClient().chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a helpful startup financial assistant. Be concise and data-driven.',
      },
      { role: 'user', content: prompt },
    ],
    temperature: 0.5,
