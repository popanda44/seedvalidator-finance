...
export async function detectAnomalies(
    ...
    const burnChange = ((currentMetrics.burnRate - historicalAvg.burnRate) / historicalAvg.burnRate) * 100;
    ...
    currentMetrics.expenses.forEach(expense => {
        if (Math.abs(expense.change) > 50) {
            anomalies.push({
                ...
                category: 'Expenses',
                actionable: true,
                suggestedAction: expense.change > 0
...
function getDemoInsights(metrics: FinancialMetrics): AIInsightsResponse {
    ...
    if (metrics.runway < 6) {
        insights.push({
            ...
            category: 'Cash Management',
            actionable: true,
            suggestedAction: 'Consider reducing non-essential expenses by 20% and accelerate fundraising efforts.'
        });
